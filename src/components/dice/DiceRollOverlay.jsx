 ...
 * Sempre chama `onComplete` ao terminar ou ao desmontar (evita peão/dado travados).
 */
export default function DiceRollOverlay({
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      if (!host) {
        const rafWait = requestAnimationFrame(start)
        cleanupInner = () => cancelAnimationFrame(rafWait)
        return
      }

      if (prefersReducedMotion()) {
        const t = setTimeout(finish, 80)
        cleanupInner = () => clearTimeout(t)
        return
      }
    ...
    return () => {
      cancelled = true
      cleanupInner()
      // Sempre libera o pipeline do App (pending ROLL), mesmo em unmount/StrictMode
      finish()
    }
  }, [open, result])
